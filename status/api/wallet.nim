...
proc storeWalletAccount(self: StatusObject, name: string, address: Address,
  ...
  path: KeyPath): WalletAccountResult {.raises: [Defect, ref AssertionError,
...
proc storeDerivedAccount(self: StatusObject, id: UUID, path: KeyPath, name,
  ...
  {.raises: [Defect, ref IOError, ref KeyError, ref OSError, SqliteError,
  UserDbError, ValueError].} =
...
proc addWalletAccount*(self: StatusObject, name, password,
  dir: string): WalletAccountResult {.raises: [Defect, Exception, ref IOError,
  ref OSError, SqliteError, UserDbError, ref ValueError].} =
